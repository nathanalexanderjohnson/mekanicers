 ...
 * @extends {Actor}
 */
export class MekanicersActor extends Actor {
  /** @override */
  prepareData() {
  ...
  prepareDerivedData() {
    ...
    const flags = actorData.flags.mekanicers || {};

    if (actorData.type === 'character' && actorData.system?.pain) {
      let woundPain = 0;
      for (const item of this.items) {
        if (item.type === 'wound') {
          woundPain += Number(item.system.pain) || 0;
        }
      }
      actorData.system.pain.value = woundPain;
    }

    if (actorData.type === 'character' && actorData.system?.carryingCapacity) {
      let totalWeight = 0;
      for (const item of this.items) {
        if (item.type === 'item') {
      ...
      actorData.system.carryingCapacity.value = totalWeight;
    }
  }
